...
namespace EmotionalBasketGame.Particles
{
    /// <summary>
    ...
    public abstract class ParticleSystem
    {
        ...
        public ParticleSystem(Game game, int maxParticles)
        {
            Game = game;
            ...
            {
                particles[i].Initialize(Vector2.Zero, 0.0f);
        ...
        /// </summary>
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch, Vector2 center, Vector2 offset, float scale)
        {
            foreach (Particle p in particles)
            {
                ...
                spriteBatch.Draw(texture, center + (p.Position - offset) * scale, null, p.Color,
                    p.Rotation, origin, p.Scale * scale, SpriteEffects.None, 0.0f);
            }
        }
        ...
        protected void AddParticles(Vector2 where)
        {
            ...
            int numParticles =
                Ink_RandomHelper.Next(minNumParticles, maxNumParticles);
        ...
        protected void AddParticles(Rectangle where)
        {
            ...
            for (int i = 0; i < numParticles && freeParticles.Count > 0; i++)
            {
                // grab a particle from the freeParticles queue, and Initialize it.
                int index = freeParticles.Dequeue();
                InitializeParticle(ref particles[index], Ink_RandomHelper.RandomPosition(where));
            }
        }
